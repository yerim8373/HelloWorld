...
//    @PreAuthorize("hasAnyRole('USER', 'ADMIN')")
    public ResponseEntity<RoomDto> makeRoom(@RequestBody RoomDto roomDto) throws OpenViduJavaClientException, OpenViduHttpException {
        // 방 번호 난수 생성
    ...
    @ApiResponses({
            ...
            @ApiResponse(code = 409, message = "방 접속 불가능 상태(GAME | FULL)"),
            @ApiResponse(code = 500, message = "서버 에러")
    })
//    @PreAuthorize("hasAnyRole('USER', 'ADMIN')")
    public ResponseEntity<RoomDto> findRoom(@RequestBody RoomDto findRoomDto) throws OpenViduJavaClientException, OpenViduHttpException {
        // 검색할 방이 존재하는지 확인
        RoomDto room = roomService.findRoom(findRoomDto);
...
//            throw new RoomNotFoundException(roomId);
        ...
        // 인원초과일 경우 (근데 우리 어플에서는 우리가 직접 넣어주기 때문에 의미 없어보인다. 그럼 findRoom 메서드 자체가 의미없는거 아닌가 이름을 나중에 바꿔야겠다.)
