...
- 포스트 조회 비동기 함수 및 호출 로직 구현
  - redux 내 포스트 관련 slice 및 asyncThunk 생성
- 포스트 API 호출 시 JWT 토큰을 인자로 전달
- 기존 더미 포스트 제거, 서버로부터 받은 실제 데이터를 화면에 출력하도록 수정
- 포스트 텍스트 내용에 있는 개행 문자가 무시되는 문제를 해결
...
- 로그아웃 후 토큰 재발행 API가 호출되는 문제 해결
  - JWT 토큰을 보유하고 있을 경우 900초에 한 번씩 재발행, 토큰이 없을 경우 ms 값 대신 `null`을 전달함으로써 해결
- 랜딩 페이지 리뉴얼
...
- 랜딩 페이지 리뉴얼
  - 세 번째 섹션에 5초 간격으로 화면이 바뀌는 애니메이션과 타이머 애니메이션 추가
  - 두 번째 섹션에 적용한 블러 효과 제거
    - `backdrop-filter` 효과로 인해 렌더링 & 애니메이션 성능이 저하되는 문제가 발생했기 때문
  - 버튼 클릭 시 콘솔 출력 이벤트 대신 실제 링크 적용
  - 네 번째 섹션 배경에 각 나라 언어 인삿말을 랜덤으로 표시
  - 마지막 섹션 임시 스타일 적용
- UCC 촬영 및 자료 제공
...
- 랜딩 페이지 리뉴얼
  - 다섯 번째 섹션의 배경으로 이야기 주제를 랜덤한 내용과 색상으로 표시
  - 여섯 번째 섹션에 배경 적용, 우측에 VIP 멤버십 비교 테이블 추가
  - 아웃트로 섹션에 애니메이션 적용
- `landingPage` 컴포넌트 앞글자를 대문자로 수정
